...
def load_prediction_assets_lgbm():
    ...
    assets = {}
    try:
        assets['model'] = joblib.load('To_Publish_Debye_Temperature_Best_On_CNN.pkl')
        
        assets['cnn_scaler'] = joblib.load('debye_cnn_scaler_on_cnn.pkl')
        assets['pca'] = joblib.load('debye_pca_on_cnn.pkl')
        assets['magpie_scaler'] = joblib.load('debye_magpie_scaler_on_cnn.pkl')
        ...
        print("[To_Publish_Debye_Temperature_Best_On_CNN.pkl, debye_cnn_scaler_on_cnn.pkl, debye_pca_on_cnn.pkl, debye_magpie_scaler_on_cnn.pkl, magpie_features.csv]")
        print(f"\nDetails: {e}")
        return None
    return assets

def predict_debye_temperature_lgbm(formula: str, npy_file_path: str, assets: dict) -> float:
    """
    Predicts the Debye Temperature (K) using the Tree-Fusion (LGBM) model.
    """
    if not assets:
    ...
    try:
        final_prediction_kelvin = assets['model'].predict(x_combined)[0]
...
if __name__ == "__main__":
    ...
    if model_assets:
        ...
        print("\n" + "="*50)
        print("Usage: python predict_debye_on_cnn.py <formula_string> <path_to_npy_file>")
        
        # Check for command line arguments
        if len(sys.argv) == 3:
            formula_from_gui = sys.argv[1]
            npy_path_from_gui = sys.argv[2]
            prediction_gui = predict_debye_temperature_lgbm(
            ...
            if not model_assets['magpie_lookup_df'].empty:
                ...
                if os.path.exists(test_npy_file):
                    predict_debye_temperature_lgbm(test_formula, test_npy_file, model_assets)
                else:
