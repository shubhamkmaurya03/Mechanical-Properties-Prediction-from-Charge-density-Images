...
# --- 1. DATA LOADING ---
print(" ==================== Loading data to generate LGBM scalers ==================== ")
try:
...
def remove_outliers_advanced(X_3d_in, X_magpie_in, y_in):
    ...
    X_3d_in = X_3d_in[y_finite_mask]; X_magpie_in = X_magpie_in[y_finite_mask]; y_in = y_in[y_finite_mask]
    ...
    if len(y_filtered) == 0 or np.std(y_filtered) == 0:
        return X_3d_in, X_magpie_in, y_in
    else:
        return X_3d_in[outlier_mask], X_magpie_in[outlier_mask], y_in[outlier_mask]
...
joblib.dump(scaler_3d, 'young_cnn_scaler_on_cnn.pkl')
print("Saved young_cnn_scaler_on_cnn.pkl")
...
joblib.dump(pca, 'young_pca_on_cnn.pkl')
print(f"Saved young_pca_on_cnn.pkl (n_components={pca.n_components_})")

# 3. Fit and Save the Magpie Scaler
scaler_magpie = StandardScaler()
scaler_magpie.fit(X_magpie_final)
joblib.dump(scaler_magpie, 'young_magpie_scaler_on_cnn.pkl')
print("Saved young_magpie_scaler_on_cnn.pkl")

print("\nDone! All scalers ('keys') for the LGBM model have been saved.")