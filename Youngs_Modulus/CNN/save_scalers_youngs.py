...
# --- 1. DATA LOADING ---
print(" =================== Loading data to generate scalers =================== ")
try:
...
joblib.dump(cnn_max_val, 'cnn_max_value_young.pkl')
print(f"Saved cnn_max_value_young.pkl (Value: {cnn_max_val})")

# 2. Save the Magpie Scaler
scaler_magpie = StandardScaler()
scaler_magpie.fit(X_magpie_final)
joblib.dump(scaler_magpie, 'magpie_scaler_young.pkl')
print("Saved magpie_scaler_young.pkl")

# 3. Save the Target (y) Scaler
scaler_y = MinMaxScaler() 
scaler_y.fit(y_final.reshape(-1, 1))
joblib.dump(scaler_y, 'y_scaler_young.pkl')
print("Saved y_scaler_young.pkl")

print("\n =================== Done! All scalers ('keys') have been saved =================== ")