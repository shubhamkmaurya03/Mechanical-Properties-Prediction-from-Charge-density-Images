...
def load_prediction_assets_on_cnn():
    ...
    try:
        ...
        print("[To_Publish_Bulk_Modulus_LGBM.pkl, bulk_pca_scaler_on_cnn.pkl, bulk_pca_model_on_cnn.pkl, bulk_magpie_scaler_on_cnn.pkl, magpie_features.csv]")
        print(f"\nDetails: {e}")
        return None
    return assets
...
if __name__ == "__main__":
    ...
    if model_assets:
        ...
        print("\n" + "="*50)
        print("Usage: python predict_bulk_on_cnn.py <formula_string> <path_to_npy_file>")
        ...
        if len(sys.argv) == 3:
            ...
            if not model_assets['magpie_lookup_df'].empty:
                ...
                if os.path.exists(test_npy_file):
                    predict_bulk_modulus_on_cnn(test_formula, test_npy_file, model_assets)
                else:
